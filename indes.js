...
bot.on("message", msg => {
    ...
    switch (cmd) {
      case "info":
        ...
        console.log("round: "+ gameInfos.round);
...
function callIt(msg) {
    ...
    gameInfos.pot += subRound ? gameInfos.call - players[gameInfos.currentPlayer].call : gameInfos.call;
...
function checkGame(msg) {
  ...
  return currPlayer >= (obj.users.length - 1);
}


function manageGame(msg){
  const playersCount = players.length;
  for(; gameInfos.currentPlayer < playersCount && players[currentPlayer].state === "out"; ++gameInfos.currentPlayer);
